...
for file, reffile in zip(files, reffiles):
    ...
    # load files
    f2 = netCDF4.Dataset(file, 'r') # file insteead of reffile
    ...
    if ("ens1" in file): # file instead of reffile
        if var != "sst" and var != "sictho" and var != "sicomo":
            ref[0,ens1count,:,:,:,:] = vargm2
            if var == "vmo" or var == "umo":
                ref2[0,ens1count,:,:,:,:] = vargm3
        else:
            ref[0,ens1count,:,:,:] = vargm2
        print(file + " done! Number "+scen+" ens1: ",ens1count)
        ens1count += 1
    if ("ens2" in file): # file instead of reffile
        if var != "sst" and var != "sictho" and var != "sicomo":
            ref[1,ens2count,:,:,:,:] = vargm2
            if var == "vmo" or var == "umo":
                ref2[1,ens2count,:,:,:,:] = vargm3
        else:
            ref[1,ens2count,:,:,:] = vargm2
        print(file + " done! Number "+scen+" ens2: ",ens2count)
        ens2count += 1
    f2.close()
...
def all_files():
    ...
    for j in range(levels):
        ...
        # calculating legend - x: -100, +5; y: 7, 80
        # limit max and min to relevant value range!
        xval = []
        yval = []
        ...
        if var == "umo" or var == "vmo":
            ...
            if var != "umo" and var != "vmo":
               ...
               #cs = plt.contour(xi, yi, gradmag,clevs, colors='k',linewidths=2)
               #plt.clabel(cs, colors = 'k', fmt = fmt, fontsize=24)
        ...
        if var == "sao":
            ...
            plt.title (scen+': ' +name + ' at depth: ' + str(depths[j+17]) + 'm')
        ...
        m.fillcontinents()
        #plt.show()
        plt.savefig(path+expname+'_'+var+'_'+scen+'_'+str(17+j)+'_'+year_begin+'_'+year_end+'.pdf',bbox_inches='tight')
